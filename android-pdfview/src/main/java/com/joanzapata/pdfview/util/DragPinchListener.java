...
public class DragPinchListener implements OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        mView = v ;
        PDFView getzoom = (PDFView) mView;
        float zoom = getzoom.getZoom();
        ...
        //TODO : new addition to handle the gesture conflicts
                if(zoom > 1 ){
                    ...
                    // allow Parent Intercept, just in case
                    ViewParent parent = mView.getParent();
                    if (parent != null) {
                ...
                if (isClick(event, lastDownX, lastDownY, event.getX(), event.getY())) {
                    ...
                    if (onDoubleTapListener != null) {
                        if (time - lastClickTime < MAX_DOUBLE_CLICK_TIME) {
                            ...
                            if(zoom == 1f){
                                // TODO for double tap from 0
                                onDoubleTapListener.onDoubleTap(event.getX(), event.getY());
                            }else{
                                //TODO for double tap from max to 0
                                onDoubleTapListener.onDoubleTap(event.getX(), event.getY());
                                lastClickTime = 0;
                            }
                        } else {
